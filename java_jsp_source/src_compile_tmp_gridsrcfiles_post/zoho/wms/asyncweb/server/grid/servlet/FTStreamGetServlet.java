...
import com.adventnet.wms.servercommon.components.fts.FileInfo;
...
public class FTStreamGetServlet extends HttpStreamServlet
{
	private static final Logger LOGGER = Logger.getLogger(FTStreamGetServlet.class.getName());
	...
	public void onHeaderCompletion(HttpRequest req, HttpResponse res) throws Exception
	{
		...
		LOGGER.log(Level.INFO, "FTS GET--> On Header Completion Successful. servletInfo={0}", servletInfo.get(req.getHeader(FTSConstants.STREAM_ID)));
		addFTServerStat(req);
	}
		...
		try
		{
			synchronized (WCPUtil.getObjLockForRegister(req.getHeader(FTSConstants.STREAM_ID)))
			{
				...
				while(req.getHttpStream().isAvailable())
				{
					...
					LOGGER.log(Level.INFO, "FTS GET--> ON DATA CHUNK CurrPos={0} endPos={1}", new Object[] {fileRWStatus.getCurrPosition(), fileRWStatus.getEndPosition()});// remove after testing
				...
				if(req.getHttpStream().isFinished())
				{
					...
					LOGGER.log(Level.INFO, "FTS GET--> Temp File written successfully. streamId={0}", streamId);
					...
					try
					{
						...
						FTStats.addFTSSeverStat(FTSConstants.STREAM_MODE.getName(FTSConstants.STREAM_MODE.GET), FTSConstants.STATUS.FTSTREAM_GET_SERVLET_STREAM_ENDED, FTSConstants.COMMUNICATION_TYPE.SERVER, remoteFile.getSourceDC(), remoteFile.getSourceCluster(), streamId, raf.length(), 0);
					} catch (Exception e) // backward compatibility issue
					{
					}

					closeRaf(raf);
					LOGGER.log(Level.INFO, "FTS GET--> RAF closed successfully. streamId={0}", streamId); //remove after debug
					...
					LOGGER.log(Level.INFO, "FTS GET--> Writing response streamId={0}", streamId); //remove after debug

					res.close();
					LOGGER.log(Level.INFO, "FTS GET--> URL Connection closed. streamId={0}", streamId);
				}
			}
		}
		catch(Exception e)
	...
	private void registerServletInfo(HttpRequest req, HttpResponse res) throws Exception
	{
		try
		{
			...
			LOGGER.log(Level.INFO, "FTS GET--> registerServletInfo. streamId={0} remoteFile={1}", new Object[] {streamId, remoteFile}); //remove after debug
			...
			{
				LOGGER.log(Level.INFO, "FTS GET--> TempFilePath is empty. tempFilePath={0}",tempFilePath);
			...
			LOGGER.log(Level.INFO, "FTS GET--> registerServletInfo Successful. streamId={0}", streamId);
			FTStats.addFTSSeverStat(FTSConstants.STREAM_MODE.getName(FTSConstants.STREAM_MODE.GET), FTSConstants.STATUS.FTSTREAM_GET_SERVLET_ON_DATA, FTSConstants.COMMUNICATION_TYPE.SERVER, remoteFile.getSourceDC(), remoteFile.getSourceCluster(), streamId, 0, 0);
		}
		catch (Exception e)
		{
		...
		LOGGER.log(Level.INFO, "FTS GET--> RandomAccessFile creadted SUCCESSFULLY. servletInfo={0}", servletInfo.get(req.getHeader(FTSConstants.STREAM_ID)));
	}
	...
	private void addFTServerStat(HttpRequest req)
	{
		try
		{
			...
			FTStats.addFTSSeverStat(FTSConstants.STREAM_MODE.getName(FTSConstants.STREAM_MODE.GET), FTSConstants.STATUS.FTSTREAM_GET_SERVLET_ON_HEADER_COMPLETION, FTSConstants.COMMUNICATION_TYPE.SERVER, remoteFile.getSourceDC(), remoteFile.getSourceCluster(), req.getHeader(FTSConstants.STREAM_ID), 0, 0);
		}
		catch (Exception e)
		{
		}
	...
	{
		LOGGER.log(Level.INFO, "FTS GET--> onOutputBufferRefill");
	}

	public void onWriteComplete(HttpRequest req, HttpResponse res)
	{
		LOGGER.log(Level.INFO, "FTS GET--> onWriteComplete");
	}

	public void onWriteFailure(HttpRequest req, HttpResponse res)
	{
		LOGGER.log(Level.WARNING, "FTS GET--> onWriteFailure");
	}

	public void service(HttpRequest req, HttpResponse res)
	{
		LOGGER.log(Level.INFO, "FTS GET--> service");
	}
